...
export async function GET() {
  ...
  try {
    ...
    const result = await zernio.connect.getConnectUrl({
      ...
      query: {
        profileId,
        redirect_url: `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/tiktok/callback?state=${encodeURIComponent(userId)}`,
      },
    });
    ...
    console.error("Zernio TikTok connect error:", err.message);
    const msg = err.message || "Failed to connect TikTok";
    return new Response(
