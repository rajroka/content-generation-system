...
<html>
  ...
  <body>
    <p>${type === "SOCIAL_CONNECTED" ? "Twitter connected! You can close this window." : "Connection failed. Please try again."}</p>
    <script>
...
export async function GET(req: Request) {
  ...
  if (error) {
    console.error("Zernio Twitter callback error:", error);
    const errorMessages: Record<string, string> = {
      access_denied: "Twitter connection was cancelled.",
      token_expired: "Twitter token expired. Please try again.",
    };
    return html("SOCIAL_CONNECT_ERROR", { error: errorMessages[error] || `Twitter connection failed: ${error}` });
  }
